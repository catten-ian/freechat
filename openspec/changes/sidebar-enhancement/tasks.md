...
- [ ] 1.1 创建组件目录结构（src/components/）
- [ ] 1.2 创建 Hooks 目录（src/hooks/）
- [ ] 1.3 创建工具函数目录（src/utils/）
- [ ] 1.4 安装依赖包（react-window, @use-gesture/react）

## 2. 数据结构扩展

- [ ] 2.1 扩展 localStorage 数据结构（添加 tags, folders, archived 字段）
- [ ] 2.2 创建 storage.js 工具函数（读写 localStorage）
- [ ] 2.3 数据迁移脚本（为现有对话添加默认值）

## 3. 搜索功能实现

- [ ] 3.1 创建 useSearch Hook（搜索逻辑 + 防抖）
- [ ] 3.2 创建 SearchBar.jsx 组件
- [ ] 3.3 实现标题搜索功能
- [ ] 3.4 实现内容搜索功能
- [ ] 3.5 搜索结果高亮显示
- [ ] 3.6 搜索历史记录功能
...
- [ ] 4.2 创建文件夹管理功能（创建、删除、重命名）
- [ ] 4.3 创建标签管理功能（创建、删除、颜色选择）
...
- [ ] 6.1 创建 ConversationCard.jsx 组件（重构现有卡片）
- [ ] 6.2 显示消息预览（最后一条消息前 30 字符）
- [ ] 6.3 显示时间戳（今天/昨天/星期/日期）
- [ ] 6.4 显示标签（彩色小圆点）
- [ ] 6.5 侧栏折叠动画（300ms 过渡）
- [ ] 6.6 空状态提示（欢迎提示/搜索无结果）
...
- [ ] 9.1 将现有 App.jsx 侧栏代码迁移到 Sidebar.jsx
- [ ] 9.2 在 App.jsx 中集成新组件
- [ ] 9.3 保持现有功能不变（兼容性测试）
- [ ] 9.4 清理旧代码

## 10. 测试与优化

- [ ] 10.1 本地功能测试（搜索、分组、批量操作）
